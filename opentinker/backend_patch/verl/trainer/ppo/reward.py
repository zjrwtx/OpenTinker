...
def get_custom_reward_fn(config: DictConfig) -> Optional[RawRewardFn]:
    """Load and return a custom reward function from external file or remote API.

    Dynamically imports a reward function from a specified file path and wraps
    it with additional keyword arguments from the configuration, or creates a
    remote API caller for HTTP-based reward functions.

    Args:
        config (dict): Configuration dictionary containing custom_reward_function
                      settings with 'path', 'name', 'reward_kwargs' for file-based,
                      or 'type', 'remote_endpoint', 'remote_api_key' for remote.
    ...
    if reward_type == "remote":
        ...
        def remote_reward_fn(data_source, solution_str, ground_truth, extra_info, **kwargs):
            ...
            - Single ground_truth (str), not ground_truths (list)
            - Single extra_info (dict), not extra_infos (list)
            """
            import requests
    ...
    function_name = reward_fn_config.get("config_name") or reward_fn_config.get("name")
