...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self._create_dataloader(train_dataset, val_dataset, collate_fn, train_sampler, skip_init=skip_dataloader_init)
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler], skip_init: bool = False):
        """
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        # ===== DEBUG LOGGING START =====
                        print("=" * 80)
                        print(f"[STANDARD] Batch state before compute_log_prob (step {self.global_steps})")
                        ...
                        print(f"  response_mask shape: {batch.batch['response_mask'].shape}")
                        print(f"  response_mask sum: {batch.batch['response_mask'].sum().item()}")
                        print(f"  response_mask mean: {batch.batch['response_mask'].float().mean().item()}")
                        print(f"  First response (first 20 tokens): {batch.batch['responses'][0, :20].tolist()}")
                        print(f"  First response_mask (first 20): {batch.batch['response_mask'][0, :20].tolist()}")
                        ...
                        # ===== DEBUG LOGGING START =====
                        print(f"[STANDARD] After compute_log_prob (step {self.global_steps})")
                        ...
                        print(f"  First entropy values (first 20): {entropys[0, :20].tolist()}")
                        print("=" * 80)
                        # ===== DEBUG LOGGING END =====
